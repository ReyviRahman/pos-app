...
<div>
    <!-- Mobile overlay -->
    <div x-show="sidebarOpen" x-cloak class="fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-sm lg:hidden" @click="sidebarOpen = false"
    ...
    <nav :class="{'translate-x-0 lg:ml-0': sidebarOpen, '-translate-x-full lg:-ml-72': !sidebarOpen}" 
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white flex-shrink-0">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
                <div class="bg-indigo-600 text-white rounded-xl w-9 h-9 flex items-center justify-center font-bold text-lg shadow-sm">
                    R
                </div>
                <span class="font-bold text-xl tracking-tight text-slate-900">SDR <span class="text-indigo-600">POS</span></span>
            </a>
            <button @click="sidebarOpen = false" class="lg:hidden p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
        ...
        <div class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium transition {{ request()->routeIs('dashboard') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:text-indigo-600 hover:bg-slate-50' }}">
        ...
        <div class="p-4 border-t border-slate-100 bg-white flex-shrink-0">
            <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl mb-3">
                <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-lg flex-shrink-0">
                    {{ strtoupper(substr(Auth::user()->name ?? Auth::user()->username ?? 'U', 0, 1)) }}
                </div>
                <div class="flex-1 overflow-hidden">
                    <div class="font-bold text-slate-800 text-sm truncate">{{ Auth::user()->name ?? Auth::user()->username }}</div>
                    <div class="text-xs text-slate-500 truncate">{{ Auth::user()->email }}</div>
                </div>
            </div>

            <div class="flex gap-2">
                <a href="{{ route('profile.edit') }}" class="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 text-sm font-semibold text-slate-600 bg-white rounded-xl border border-slate-200 hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-sm">
                ...
                <form method="POST" action="{{ route('logout') }}" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full h-full flex items-center justify-center gap-2 px-3 py-2.5 text-sm font-semibold text-rose-600 bg-rose-50 rounded-xl hover:bg-rose-100 transition-colors shadow-sm">
