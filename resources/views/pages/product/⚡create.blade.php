...
{
    // Properti untuk Model Product
    public string $name = '';
    public string $price = '';

    // Array untuk menyimpan baris bahan-bahan (tabel pivot)
    public array $menuIngredients = [];

    // --- Properti untuk Modal Ingredient Baru ---
    ...
    {
        // Tambahkan satu baris kosong secara default saat komponen dimuat
        $this->addIngredient();
    }

    // Fungsi untuk menambah baris input bahan baku baru
    ...
    // Fungsi untuk menghapus baris input bahan baku
    public function removeIngredient($index)
    {
        unset($this->menuIngredients[$index]);
        $this->menuIngredients = array_values($this->menuIngredients); // Re-index array
    }
    ...
    public function save()
    {
        ...
        DB::transaction(function () {
            ...
            // Siapkan array data untuk dimasukkan ke tabel pivot MenuIngredient
            $pivotData = [];
            ...
            $product->ingredients()->attach($pivotData);
        });
        ...
        return redirect()->route('product.index')->with('success', 'Produk dan resep bahan berhasil disimpan!');
    }

    // --- Fungsi Modal Ingredient ---
    ...
    {
        // Validasi bahan baku baru
        $this->validate([
            'newIngredientName' => 'required|string|max:255',
            'newIngredientUnit' => 'required|string|max:50', // contoh: kg, gram, pcs, liter
        ]);
    ...
    // Mengirim data master Ingredient ke View untuk dropdown
...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-8 relative">
    ...
    <form wire:submit="save">
        {{-- Bagian Data Produk --}}
        <div class="mb-8">
            <h3 class="text-xl font-bold text-gray-800 border-b pb-2 mb-4">Data Produk Baru</h3>
                ...
                @foreach($menuIngredients as $index => $item)
                    <div class="flex flex-col md:flex-row gap-4 items-start md:items-center bg-gray-50 p-4 rounded-md border border-gray-200">
                        ...
                        <div class="w-full md:w-auto mt-4 md:mt-5">
                            ...
                            @else
                                <div class="w-full md:w-[70px]"></div> {{-- Spacer --}}
                            @endif
                        </div>
                    </div>
                @endforeach
